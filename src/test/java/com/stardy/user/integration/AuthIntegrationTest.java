package com.stardy.user.integration;
...
public class AuthIntegrationTest {
    ...
    void reissueWithStolenToken() {
        ...
        BaseException exception = assertThrows(BaseException.class, () -> authService.reissueToken(firstRefreshToken));
    ...
    private User createUserWithRole(String email, String roleName, String nickName) {
        Role role = roleRepository.findByRoleId(roleName)
                .orElseGet(() -> roleRepository.save(new Role(roleName, roleDisplayName(roleName))));
        ...
        User user = new User(
                ...
                Map.of("job", "teacher",
                        "regions", List.of("Seoul", "Gyenoggi"),
                        "subjects", List.of("Korean, English")),
                role,
                "ACTIVE"
        );
        return user;
